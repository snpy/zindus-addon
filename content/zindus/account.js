...
Account.prototype = {
    make_getter:      function (key) {
        var context = this;
        return function () {
            return context.get(eAccount[key]);
        }
    },
    make_setter:      function (key) {
        var context = this;
        return function (value) {
            context.set(eAccount[key], value);
        }
    },
    get:              function (key) {
    ...
    save:             function () {
        zinAssertAndLog(this.sourceid, function () {
...
var AccountStatic = {
    ...
    unique_key:            function (format_xx, url, username) {
        ...
        switch (format_xx) {
            case FORMAT_LD:
                ret = hyphenate(":", Account.Ldap, url, username);
                break;
            case FORMAT_ZM:
                ret = hyphenate(":", Account.Zimbra, url, username);
                break;
            case FORMAT_GD:
                ret = hyphenate(":", Account.Google, username);
                break;
            default:
                zinAssertAndLog(false, "mismatched case: " + format_xx);
        }
        return ret;
    },
