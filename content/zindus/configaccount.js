    ...
    initialiseView:           function () {
        ...
        function initialise_elements(account) {
            with (ConfigAccountStatic) {
                ...
                v = account.gd_gr_as_ab;
                setRadio("ca-gd-gr-as-ab-radiogroup", self.m_gd_gr_as_ab_bimap, v ? v : 'false');
                v = account.gd_suggested;
                setRadio("ca-gd-suggested-radiogroup", self.m_gd_suggested_bimap, v ? v : 'include');
                v = account.gd_sync_with;
                setRadio("ca-gd-syncwith-radiogroup", self.m_gd_sync_with_bimap, v ? v : 'pab');
                v = account.zm_sync_gal_enabled;
                setRadio("ca-zm-gal-radiogroup", self.m_gal_radio_bimap, v ? v : 'if-fewer');
            }
        }
        ...
        if (account) {
            ...
            if (this.m_payload_caller.m_format) {
                switch (this.m_payload_caller.m_format) {
                    case Account.Google:
                        break; // initialised above
                    case Account.Zimbra:
                        url = "";
                        format = Account.Zimbra;
                        break;
                    case 'Share':
                        url = ConfigAccountStatic.m_share_service_url;
                        format = Account.Zimbra;
                        break;
                    default:
                        zinAssertAndLog(false, this.m_payload_caller.m_format);
                }
            }
