    ...
    zm_street_normalise:             function (format_to, format_from, properties_from, key_from, a_normalised_street) {
        ...
        switch (format_from) {
            case FORMAT_TB:
                switch (key_from) {
                    case "HomeAddress":
                        a_normalised_street["home"][0] = properties_from[key_from];
                        break;
                    case "WorkAddress":
                        a_normalised_street["work"][0] = properties_from[key_from];
                        break;
                    case "HomeAddress2":
                        this.lineTwoFromCommaSeparated(properties_from, key_from, a_normalised_street, "home");
                        break;
                    case "WorkAddress2":
                        this.lineTwoFromCommaSeparated(properties_from, key_from, a_normalised_street, "work");
                        break;
                    default:
                        zinAssert(false);
                }
            ...
            case FORMAT_ZM:
                switch (key_from) {
                    case "homeStreet":
                        this.lineFromNewlineSeparated(properties_from, key_from, a_normalised_street, "home");
                        break;
                    case "workStreet":
                        this.lineFromNewlineSeparated(properties_from, key_from, a_normalised_street, "work");
                        break;
                    default:
                        zinAssert(false);
                }
    ...
    lineFromNewlineSeparated:        function (properties_from, key_from, a_line, type) {
        ...
        for (var i = 0; i < a.length; i++)
            if (i == 0) {
                ...
                a_line[type][i] = a[i].replace(/,/, " ");
            } // can't allow commas in line 2 and onwards
    },
...
//
    ...
    tb_birthday_output:              function (format_to, properties_to, a_normalised_tb_birthday) {
        ...
        function pad_yyyy(x) {
            return String(empty_year + x).slice(-4);
        }
