...
SyncFsmObserver.prototype = {
    ...
    progressReportOnSource: function () {
        let a = arguments;
        zinAssertAndLog((a.length == 2) || (typeof(a[2]) == 'number'), function () {
            return "length: " + a.length + " typeof: " + typeof(a[2]);
        });
    ...
    updateState:            function (fsmstate, a_states) {
        ...
        if (fsmstate.newstate && ('count' in a_states[fsmstate.newstate])) { // fsmstate.newstate == null when oldstate == 'final'
            ...
            switch (fsmstate.newstate) {
                ...
                case 'stAuthPreAuth':
                case 'stAuth':
                    this.progressReportOnSource(context.state.sourceid_pr, "remote.auth");
                    break;
                case 'stLoad':
                    this.progressReportOn("load");
                    break;
                case 'stGetInfo':
                    this.progressReportOnSource(context.state.sourceid_pr, "account.info");
                    break;
                case 'stGetAccountInfo':
                    this.progressReportOnSource(context.state.sourceid_pr, "account.info");
                    break;
                case 'stSync':
                case 'stSyncResponse':
                ...
                case 'stGetContactGd2':
                    this.progressReportOnSource(context.state.sourceid_pr, "remote.sync");
                    break;
                case 'stGalSync':
                case 'stGalCommit':
                    this.progressReportOnSource(context.state.sourceid_pr, "get.gal");
                    break;
                case 'stLoadTb':
                    this.progressReportOnSource(context.state.sourceid_tb, "load");
                    break;
                case 'stUpdateTb':
                    this.progressReportOnSource(context.state.sourceid_tb, "put.one");
                    break;
                case 'stUpdateCleanup':
                    this.progressReportOn("saving");
                    break;
                ...
                case 'stUpdateGd':
                {
                    ...
                    if (!context.state.m_suo_generator) {
                        max_suos = Suo.count(context.state.aSuo, function (sourceid, bucket) {
                            return sourceid == context.state.sourceid_pr;
                        });
                    }
                ...
                case 'final':
                    ...
                    if (fsmstate.event == 'evCancel') {
                        ...
                        zinAssert(false);
                    } // ensure that all cases are covered above
