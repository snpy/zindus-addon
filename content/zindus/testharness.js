...
TestHarness.prototype.testSuo = function () {
    ...
    for (suo in it.iterator(function (sourceid, bucket) {
        return true;
    }))
        str += suo.toString();
    ...
    generator = it.generator(function (sourceid, bucket) {
        return true;
    });
    ...
    var key;
    for ([key, suo] in it.iterator(function () {
        return true;
    })) {
        str += suo.toString();
...
TestHarness.prototype.testContactConverterGdPostalAddress = function () {
    ...
    zinAssert(gd_address.ns
    ::
    street.length() == 2 && zinTrim(String(gd_address.ns
    ::
    street[0]
    )) ==
    this.m_street1
    && zinTrim(String(gd_address.ns
    ::
    street[1]
    )) ==
    this.m_street2
...
TestHarness.prototype.testFolderConverterSuiteThree = function (converter, email_address) {
    function munge(x) {
        return "zindus/" + email_address + GD_GR_AS_AB_SEPARATOR + x;
    }
...
TestHarness.prototype.testContactGoogle1 = function () {
    ...
    if (false) {
        str = new String(xml.nsAtom
    ::
        xxx
    )
        ;
        b = Boolean(xml.nsAtom
    ::
        ie
    )
        ;
...
TestHarness.prototype.testContactGoogle2 = function () {
    ...
    function match(contact, properties, meta) {
        ...
        contact.mode(mode);
        context.matchContactGoogle(contact, properties, meta, mode & ContactGoogle.ePostal.kEnabled);
...
rel = "http://schemas.google.com/g/2005#work" > < / gd
:
postalAddress >;
else
contact.m_entry. * +=
...
rel = 'http://schemas.google.com/g/2005#work' > < gd
:
formattedAddress > < /gd:formattedAddress></g
...
TestHarness.prototype.testContactGoogleIssue151 = function () {
    ...
    let abCard = addressbook.addCard(uri, {
        PrimaryEmail: "111-test@example.com",
        DisplayName:  "111 test",
        Company:      company
    }, {});
...
;
zinAssert(a_keys['email1'].
@
address == 'x' && a_keys['email2'].
@
address == 'y'
)
;
...
zinAssert(a_keys['email1'].
@
address == 'z' && a_keys['email2'].
@
address == 'x' && aToLength(a_keys) == 2
)
;
...
TestHarness.prototype.testGdAddressConverter = function () {
    ...
    var out = new Object();
    var a_fields = {
        "Address": "Apartment 2", "Address2": "123 Collins st", "City": "Melbourne",
...
TestHarness.prototype.setupFixtureGdPostalAddress = function () {
    ...
    this.m_address_as_e4x =
    <
    address > < street > {this.m_street1
} < / street >
< street > {this.m_street2
} <
/street> <city> Melbourne </
city > < state > Vic < /state> <postcode> 3000 </
postcode >
< country > Australia < /country> <otheraddr> {this.m_otheraddr} </
otheraddr > < / address >;
...
TestHarness.prototype.createGoogleRuleViolation = function () {
    ...
    this.addCardTb2({
        PrimaryEmail: "222-test@example.com",
        DisplayName:  "222 test",
        Notes:        "222-test line one\nline two"
    });
    this.addCardTb2({
        PrimaryEmail: "222-test@example.com",
        DisplayName:  "222 test",
...
TestHarness.prototype.testExitStatusMessages = function () {
    ...
    var a = {
        ...
        'failon.unable.to.update.thunderbird': {
            'trailer': stringBundleString("status.failon.unable.to.update.thunderbird.detail1",
                ['fred'])
        },
        'failon.no.xpath':                     {},
        'failon.no.tbpre':                     {},
        'failon.no.pab.2':                     {
            'trailer': stringBundleString("text.file.bug", [url('reporting-bugs')]),
            'arg':     ['Thunderbird']
        },
        ...
        'failon.unauthorized':                 {},
        'failon.auth':                         {
            'trailer': stringBundleString("text.http.status.code", [403]),
            'arg':     ['Thunderbird']
        },
        '':                                    {}
    };
...
TestHarness.prototype.testPerformanceLoggingStyles = function () {
    ...
    if (false) {
        ...
        stopwatch.mark("starts: make one big string then log it: " + (stopwatch.elapsed() - last));
        last = stopwatch.elapsed();
    ...
    if (true) {
        ...
        stopwatch.mark("starts: log via LogAppenderHoldOpen: " + (stopwatch.elapsed() - last));
        last = stopwatch.elapsed();
        last = 0;
        ...
        stopwatch.mark("before loop with buflogger: " + (stopwatch.elapsed() - last));
        last = stopwatch.elapsed();
        ...
        stopwatch.mark("ends: after loop with buflogger: " + (stopwatch.elapsed() - last));
        last = stopwatch.elapsed();
    }
