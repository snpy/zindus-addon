...
function SyncFsmGd(id_fsm) {
    SyncFsm.call(this);
}
function SyncFsmGdAuth(id_fsm) {
    SyncFsmGd.call(this);
}
function SyncFsmZm(id_fsm) {
    SyncFsm.call(this);
}
...
SyncFsmZm.prototype.initialiseFsm = function () {
    var transitions = {
        ...
        stGetContactZm:   {
            evCancel:      'final',
            evNext:        'stGalConsider',
            evHttpRequest: 'stHttpRequest',
            evRepeat:      'stGetContactZm'
        },
...
SyncFsmGd.prototype.initialiseFsm = function () {
    var transitions = {
        ...
        stGetGroupsGd2:   {
            evCancel:        'final',
            evNext:          'stGetContactGd1',
            evRename:        'stRenameGroups',
            evLackIntegrity: 'final'
        },
        stRenameGroups:   {
            evCancel:      'final',
        ...
        stGetGroupPuGd:   {
            evCancel:      'final',
            evNext:        'stGetContactPuGd',
            evHttpRequest: 'stHttpRequest',
            evRepeat:      'stGetGroupPuGd'
        },
        stGetContactPuGd: {
            evCancel:      'final',
            evNext:        'stUpdateGd',
            evHttpRequest: 'stHttpRequest',
            evRepeat:      'stGetContactPuGd'
        },
        stUpdateGd:       {
            evCancel:        'final',
            evNext:          'stUpdateGdPhoto',
            evHttpRequest:   'stHttpRequest',
            evRepeat:        'stUpdateGd',
            evLackIntegrity: 'final'
        },
...
SyncFsm.prototype.entryActionLoadGenerator = function (state) {
    ...
    function zfitem(zfc, zfc_key, zfi_key) {
        return zfc.isPresent(zfc_key) ? zfc.get(zfc_key).getOrNull(zfi_key) : null;
    }
...
SyncFsm.prototype.isConsistentDataStoreGenerator = function (result, aReverse) {
    function do_debug(self, nn) {
        self.state.stopwatch.mark("isConsistentDataStore: " + nn + ": is_consistent: " + result.is_consistent);
    }
            ...
            if (sourceid in self.state.sources) {
                ...
                if (self.state.sources[sourceid]['format'] == FORMAT_GD) {
                    zinAssertAndLog(self.zfc(sourceid).isPresent(luid),
                        function () {
                            return " gid: " + gid + " sourceid: " + sourceid + " luid: " + luid +
...
SyncFsm.prototype.isConsistentSourcesGenerator = function (result) {
    ...
    var functor_foreach_luid = {
        ...
        run:   function (zfi) {
            ...
            //
            if (result.is_consistent && !zfi.isPresent(FeedItem.ATTR_XGID) && SyncFsm.isRelevantToGid(zfc, luid) && !(luid in this.state.aReverseGid[sourceid])) {
...
SyncFsm.prototype.initialiseTbAddressbookGenerator = function () {
    ...
    switch (AppInfo.ab_version()) {
        case AppInfo.eApp.firefox:
            a_ids_to_remove = null;
            break;
        case AppInfo.eApp.thunderbird2:
            a_ids_to_remove = newObject(TBCARD_ATTRIBUTE_LUID, "");
            break;
        default:
            a_ids_to_remove = newObject(TBCARD_ATTRIBUTE_LUID, "", TBCARD_ATTRIBUTE_LUID_ITER, "");
            break;
    }
...
SyncFsm.prototype.exitActionSelectSoapUrl = function (state, event) {
    ...
    if (is_tested || this.state.m_http.m_faultcode == "service.UNKNOWN_DOCUMENT") {
        ...
        if (!is_tested) {
            this.state.a_zm_tested_soapurls.push(this.state.suggestedSoapURL);
        } // array is passed to hyphenate, which wants a string
...
SyncFsm.prototype.entryActionSyncResponse = function (state, event, continuation) {
    ...
    if (!this.state.m_http.response()) {
        ...
        if (node && change.acct) {
            a_foreign_folder_present = new Object();
        } // turns on change detection for folders in foreign accounts
            ...
            run:   function (doc, node) {
                ...
                if (!('id' in attribute) || !(FeedItem.ATTR_L in attribute)) {
                    ...
                    if (!zfcZm.isPresent(key)) {
                        ...
                        } // TODO here for bug #252 - remove July 2010
                    }
...
SyncFsm.prototype.entryActionGetContactPuZm = function (state, event, continuation) {
    if (!this.state.is_done_get_contacts_pu) {
        ...
        var fn = function (sourceid, bucket) {
            return (sourceid == sourceid_pr) && (bucket == (Suo.DEL | FeedItem.TYPE_CN));
        }
            ...
            if (this.state.aContact[i].key() == key) {
                if (functor.a[i].isMailList()) {
                    ...
                    if (this.zfcPr().isPresent(key)) {
                        this.zfcPr().get(key).set(functor.a[i].attribute);
                    }// update existing item
                    else {
                        this.zfcPr().set(new FeedItem(FeedItem.TYPE_CN, functor.a[i].attribute));
                    }  // add new item
                }
            }
            else {
...
SyncFsm.prototype.entryActionGalCommit = function (state, event, continuation) {
    ...
    if (this.account().zm_sync_gal_enabled == "no") {
        ...
        if (this.state.SyncGalTokenInRequest == null && this.state.SyncGalTokenInResponse != null) {
            ...
            isGalEnabled = !zfcLastSync.get(sourceid_pr).isPresent('SyncGalEnabledRecheck');
        } // stay as we are
...
SyncFsm.prototype.entryActionLoadTbGenerator = function (state) {
    ...
    if (this.formatPr() == FORMAT_ZM) {
        ...
        if (this.is_slow_sync()) {
            this.loadTbDeleteReadOnlySharedAddresbooks();
        }      // 3. ensure that we don't try to update read-only addressbooks on the server
    }
    else if (this.formatPr() == FORMAT_GD) {
...
SyncFsmGd.prototype.gdAddressbookName = function (arg) {
    ...
    switch (arg) {
        case 'public':
            ret = this.account().gd_sync_with == 'zg' ? name_when_zg : this.state.m_addressbook.getPabName();
            break;
        case 'internal':
            ret = this.account().gd_sync_with == 'zg' ? name_when_zg : TB_PAB;
            break;
        case 'zg':
            ret = name_when_zg;
            break;
            ...
            run:   function (uri, item) {
                ...
                if (isInTopLevelFolder) {
                    ...
                    if (ab_has_uuids) {
                        ...
                        is_new_card = (!(id > AUTO_INCREMENT_STARTS_AT));
                    } // id == null ==> not present or
...
SyncFsm.prototype.setTwin = function (sourceid, luid, sourceid_tb, luid_tb, reverse) {
    ...
    zinAssertAndLog(luid_tb in reverse[sourceid_tb], function () {
        return "sourceid_tb: " + sourceid_tb + " luid_tb: " + luid_tb;
    });
...
SyncFsm.prototype.updateGidFromSourcesGenerator = function () {
            ...
            if (!SyncFsm.isOfInterest(zfc, luid)) {
                ...
                if (zfi.type() == FeedItem.TYPE_FL || zfi.type() == FeedItem.TYPE_SF || zfi.type() == FeedItem.TYPE_GG) {
                    ...
                    zinAssertAndLog((zfi.type() != FeedItem.TYPE_FL) || !zfi.isForeign(),
                        function () {
                            "foreign folder? zfi: " + zfi.toString();
                        });
    ...
    function SuggestedGenerator(functor, chunk_size) {
        ...
        pass_number = 1;
        generator = zfc.forEachGenerator(functorx, chunk_size);
        ...
        run: function (zfi) {
            if (zfi.isPresent(FeedItem.ATTR_PRES)) {
                ...
                zinAssertAndLog(false, function () {
                    return "Found a gid not referenced by any sourceid/luids.  zfi: " + zfi.toString();
                });
            }
            return true;
        }
            ...
            if (zfi_gd &&
                ...
                if (zfi.isPresent(sourceid_tb)) { // Thunderbird and Google contact are twins
                    ...
                    if ((contact.groups.length == 0) && self.isTwin(sourceid_tb, sourceid_gd, luid_tb, luid_gd_au, self.contact_converter())) {
                        ...
                        zinAssertAndLog(!self.isGidBackdated(zfi), function () {
                            return zfi.toString();
                        });
    ...
    var functor_foreach_gid = {
        run:                                 function (zfi) {
            if (zfi.isPresent(sourceid_tb) && zfi.isPresent(sourceid_gd) && zfcTb.get(zfi.get(sourceid_tb)).type() == FeedItem.TYPE_CN) {
                ...
                zinAssertAndLog(luid_gd_au in self.state.a_gd_contact, function () {
                    return "luid_gd_au=" + luid_gd_au;
                });
    ...
    var functor_foreach_gid = {
        run:        function (zfi) {
            if (zfi.isPresent(sourceid_tb) && zfi.isPresent(sourceid_pr) && zfcTb.get(zfi.get(sourceid_tb)).type() == FeedItem.TYPE_CN) {
                ...
                if (the_winner == 'both') {
                    the_winner = 'none';
                } // if there are photos on both sides, don't mess with it
...
SyncFsm.prototype.isGidBackdated = function (zfi) {
    ...
    function is_zfc_backdated(sourceid, luid) {
        ...
        switch (format) {
            case FORMAT_TB:
                ret = zfi.get(FeedItem.ATTR_CS) == TBCARD_CHECKSUM_BACKWARDS;
                break;
            case FORMAT_ZM:
        ...
        compare: function (gid) {
            ...
            var functor_each_luid_in_gid = {
                run: function (sourceid, luid) {
                    ...
                    zinAssertAndLog(zfi, function () {
                        return "sourceid/luid: " + sourceid + "/" + luid;
                    });
                    ...
                    zinAssertAndLog(zfi.isPresent(FeedItem.ATTR_KEY), // debugging bug #253
                        function () {
                            return sourceid + "/" + luid + " zfi: " + zfi.toString() + " msg: " + msg;
                        });
            ...
            zinAssertAndLog(cNeverSynced == 0 || cNeverSynced == 1,
                function () {
                    return "gid=" + gid + " cNeverSynced: " + cNeverSynced + " " + buildgcs_msg + "\n" + msg;
                });

            if (cNeverSynced == 1) {
                zinAssertAndLog(cVerMatchesGid == 0 && cChangeOfNote == 0,
                    function () {
                        return "gid=" + gid + " " + buildgcs_msg + "\n" + msg;
                    });
                ...
                if (cVerMatchesGid == 1) {
                    ...
                    //
                    zinAssertAndLog((sourceid_tb in aVerMatchesGid), function () {
                        return buildgcs_msg + "\n" + msg;
                    });
        ...
        if (zfcGid.get(gid).isPresent(sourceid_tb)) {
            ...
            if (!this.zfcTb().get(luid_tb).isPresent(FeedItem.ATTR_DEL)) {
                ...
                zinAssertAndLog(zfcGid.isPresent(gid_l),
                    function () {
                        return "luid_gd_au: " + luid_gd_au + " luid_tb_l: " + luid_tb_l + " gid_l: " + gid_l;
                    });
...
SyncFsmGd.prototype.suoGdTweakCiOps = function () {
    ...
    let id_gd_suggested = this.state.gd_cc_meta.find('name', ContactGoogle.eSystemGroup.Suggested, 'luid_gd')
    let sourceid_pr = this.state.sourceid_pr;
    let fn_add = function (sourceid, bucket) {
        return (sourceid == sourceid_pr) && (bucket == (Suo.ADD | FeedItem.TYPE_CN));
    }
    let fn_mod = function (sourceid, bucket) {
        return (sourceid == sourceid_pr) && (bucket == (Suo.MOD | FeedItem.TYPE_CN));
    }
    let fn_del = function (sourceid, bucket) {
        return (sourceid == sourceid_pr) && (bucket == (Suo.DEL | FeedItem.TYPE_CN));
    }
...
SyncFsm.prototype.suoRemoveContactOpsWhenFolderIsBeingDeleted = function () {
    let fn = function (sourceid, bucket) {
        return bucket & FeedItem.TYPE_CN;
    }
    let a_suo_to_delete = new Array();
    let msg = "";
    let key, suo;
    ...
    for ([key, suo] in this.state.m_suo_iterator.iterator(fn)) {
        if (key.bucket & Suo.ADD) {
            ...
            zinAssertAndLog(!(gid_l_winner in this.state.a_folders_deleted), function () {
                return key.toString();
            });
        }
        else {
...
SyncFsm.prototype.suoTestForConflictingUpdateOperations = function () {
    ...
    var msg = "";
    var fn = function (sourceid, bucket) {
        return bucket & (FeedItem.TYPE_FL | FeedItem.TYPE_SF | FeedItem.TYPE_GG);
    }
    var key, suo;
...
SyncFsm.prototype.shortLabelForLuid = function (sourceid, luid) {
    ...
    if (type == FeedItem.TYPE_FL || type == FeedItem.TYPE_SF) {
        ...
        zinAssertAndLog(type == FeedItem.TYPE_CN, function () {
            return "sourceid: " + sourceid + " luid: " + luid + " type: " + type;
        });
...
SyncFsm.sharedFoldersUpdateAttributes = function (zfc, luid_link) {
    ...
    zinAssertAndLog(zfiLn.isPresent(FeedItem.ATTR_SKEY), function () {
        return "luid_link=" + luid_link + " zfiLn: " + zfiLn.toString();
    });

    var zfiSf = zfc.get(zfiLn.get(FeedItem.ATTR_SKEY));

    zinAssertAndLog(zfiSf.isPresent(FeedItem.ATTR_FKEY), function () {
        return "zfiSf: " + zfiSf.toString();
    });
...
SyncFsmGd.prototype.entryActionConfirmUI = function (state, event, continuation) {
    ...
    function do_count(zfc, functor, format) {
        zfc.forEach(functor);
        let fn = function (sourceid, bucket) {
            return (self.state.sources[sourceid]['format'] == format) && (bucket & Suo.DEL);
        }
    ...
    let functor_gd = {
        ...
        run:   function (zfi) {
            if (zfi.type() == FeedItem.TYPE_CN &&
                zfi.styp() == FeedItem.eStyp.gdci && !zfi.isPresent(FeedItem.ATTR_DEL)) {
    ...
    let sourceid_pr = this.state.sourceid_pr;
    let sourceid_tb = this.state.sourceid_tb;
    let fn_add_gd = function (sourceid, bucket) {
        return (sourceid == sourceid_pr) && (bucket == (Suo.ADD | FeedItem.TYPE_CN));
    }
    let fn_add_tb = function (sourceid, bucket) {
        return (sourceid == sourceid_tb) && (bucket == (Suo.ADD | FeedItem.TYPE_CN));
    }
    ...
    if (this.state.m_is_attended && is_show_again && this.is_slow_sync() && c_at_tb != 0 && c_at_gd != 0 && !(c_added_to_tb == 0 && c_added_to_gd == 0)) {
...
SyncFsm.prototype.entryActionUpdateTbGenerator = function (state) {
    ...
    var addressbook = this.state.m_addressbook;
    var fn = function (sourceid, bucket) {
        return (sourceid == self.state.sourceid_tb);
    }
        ...
        for (var suo in this.state.m_suo_iterator.iterator(fn)) {
            ...
            with (x) {
                ...
                function set_photo_properties(properties) {
                    ...
                    zinAssertAndLog(luid_winner_au in self.state.a_gd_contact, function () {
                        return "luid_winner_au: " + luid_winner_au;
                    });
...
SyncFsm.prototype.entryActionUpdateZm = function (state, event, continuation) {
    ...
    var self = this;
    var fn = function (sourceid, bucket) {
        return (self.state.sources[sourceid]['format'] == FORMAT_ZM);
    }
    var remote = null;
    ...
    if (!this.state.stopFailCode && this.state.m_a_remote_update_package.length == 0) {
        ...
        this.state.m_a_remote_update_package.length < MAX_BATCH_SIZE && !key_suo.isEqual(this.state.m_suo_last_in_bucket[key_suo.bucket]));
    }
    ...
    if (this.state.m_a_remote_update_package.length != 0) {
        ...
        if (this.state.m_a_remote_update_package[0].remote.method in ZmSoapDocument.complexMethod) {
            ...
            (zid == this.state.m_a_remote_update_package[i].remote.zid) && !(this.state.m_a_remote_update_package[i].remote.method in ZmSoapDocument.complexMethod); i++)
                ;
        ...
        run:   function (doc, node) {
            ...
            if (!('id' in attribute)) {
                ...
                if ('luid_target' in remote) {
                    zfiTarget = zfcTarget.get(remote.luid_target);
                }  // used in MOD | TYPE_FL

                if (key_suo.bucket == (Suo.DEL | FeedItem.TYPE_FL) ||
                    (key_suo.bucket == (Suo.DEL | FeedItem.TYPE_LN) )) {
                    zfiTarget.set(FeedItem.ATTR_L, ZM_ID_FOLDER_TRASH);
                }// the folder got "renamed" into trash so fake the l attribute
                else {
...
SyncFsm.prototype.entryActionUpdateGd = function (state, event, continuation) {
    ...
    let self = this;
    let fn = function (sourceid, bucket) {
        return (self.state.sources[sourceid]['format'] == FORMAT_GD);
    }
    let remote = newObject('method', null);
...
SyncFsmGd.prototype.exitActionUpdateGd = function (state, event) {
    ...
    function modify_zfi(g) {
        ...
        zinAssertAndLog(zfiTarget, function () {
            return suo.toString();
        });
        zinAssertAndLog(zfiTarget.key() == g.meta.id, function () {
            return " key: " + zfiTarget.key() + " g.meta.id: " + g.meta.id;
        });
        ...
        if (this.state.m_gd_progress_count < this.state.a_gd_photo_to_put.length) {
            ...
            with (x) {
                ...
                if (suo.m_photo_op == 'mod') {
                    ...
                    if (zfi_au.isPresent(FeedItem.ATTR_ETAG) && !zfi_au.isPresent(FeedItem.ATTR_GDME) &&
...
SyncFsmGd.prototype.exitActionUpdateGdPhoto = function (state, event) {
    ...
    if (http.is_http_status(HTTP_STATUS_2xx)) {
        ...
        with (x) {
            ...
            let luid_tb_l = zfiWinner.get(FeedItem.ATTR_L);
            zinAssertAndLog(luid_tb_l in aReverseGid[sourceid_tb], luid_tb_l);
...
SyncFsm.prototype.getContactFromLuid = function (sourceid, luid, format_to) {
    ...
    zinAssertAndLog(zfi && zfi.type() == FeedItem.TYPE_CN, function () {
        "sourceid: " + sourceid + " luid=" + luid;
    });
...
SyncFsm.prototype.entryActionUpdateCleanupGenerator = function (state) {
    ...
    if (!this.state.stopFailCode) {
        ...
        if (!this.state.stopFailCode) {
            for (var suo in this.state.m_suo_iterator.iterator(function () {
                return true;
            }))
                if (!suo.is_processed) {
...
SyncFsm.prototype.zfcTb = function () {
    return this.zfc(this.state.sourceid_tb);
}
SyncFsm.prototype.zfcPr = function () {
    return this.zfc(this.state.sourceid_pr);
}
SyncFsm.prototype.formatPr = function () {
    return this.state.sources[this.state.sourceid_pr]['format'];
}
SyncFsm.prototype.account = function () {
    return this.state.sources[this.state.sourceid_pr]['account'];
}
SyncFsm.prototype.getIntPref = function (key) {
    var p = preferences();
    return p.getIntPref(p.branch(), key);
}
SyncFsm.prototype.getCharPref = function (key) {
    var p = preferences();
    return p.getCharPref(p.branch(), key);
}
...
SyncFsm.prototype.debug = function (str) {
    ...
    this.debug_do(str, false);
}
SyncFsm.prototype.debug_continue = function (str) {
    zinAssert(arguments.length == 1);
    this.debug_do(str, true);
}
        ...
        if (zfc.isPresent(ret)) {
            ...
            if (zfi.isForeign()) {
                if (zfi.type() == FeedItem.TYPE_FL) {
                    ...
                    zinAssertAndLog(false, function () {
                        return "something is wrong: zfi: " + zfi.toString() + " key: " + key;
                    });
                }
            }
        }
...
SyncFsm.newSyncFsm.a_map["twoway" + FORMAT_GD] = {
    id_fsm: Maestro.FSM_ID_GD_TWOWAY, syncfsm: function () {
        return new SyncFsmGd();
    }
};
SyncFsm.newSyncFsm.a_map["twoway" + FORMAT_ZM] = {
    id_fsm: Maestro.FSM_ID_ZM_TWOWAY, syncfsm: function () {
        return new SyncFsmZm();
    }
};
SyncFsm.newSyncFsm.a_map["authonly" + FORMAT_GD] = {
    id_fsm: Maestro.FSM_ID_GD_AUTHONLY, syncfsm: function () {
        return new SyncFsmGdAuth();
    }
};
SyncFsm.newSyncFsm.a_map["authonly" + FORMAT_ZM] = {
    id_fsm: Maestro.FSM_ID_ZM_AUTHONLY, syncfsm: function () {
        return new SyncFsmZm();
    }
};
...
SyncFsm.prototype.entryActionHttpResponse = function (state, event, continuation) {
    ...
    zinAssertAndLog(http.isPostResponse(), function () {
...
HttpStateZm.prototype.failCode = function () {
    ...
    zinAssertAndLog(this.m_xhr && this.isFailed(), function () {
        return "HttpState: " + this.toString();
    });

    if (this.is_cancelled) {
        ret = 'failon.cancel';
    } else if (this.is_mismatched_response) {
        ret = 'failon.mismatched.response';
    } else if (this.m_fault_element_xml != null) {
        ret = 'failon.fault';
        ...
        ret = 'failon.unexpected';
    }  // this really is unknown
...
HttpStateZm.prototype.handleResponse = function () {
    ...
    if (this.is_cancelled) {
        ...
        if (node) {
            nextEvent = 'evNext';
        }// we found a BlahResponse element - all is well
        else {
...
HttpStateGd.prototype.failCode = function () {
    ...
    if (this.is_cancelled) {
        ret = 'failon.cancel';
    } else if (this.is_http_status(HTTP_STATUS_401_UNAUTHORIZED)) {
        ret = 'failon.unauthorized';
            ...
            if (zfcPr.isPresent(id)) {
                ...
                if ('etag' in contact.photo) {
                    if (!zfi.isPresent(FeedItem.ATTR_ETAG) ||
                        ...
                        zfi.isPresent(FeedItem.ATTR_GDME) || !SyncFsmGd.gd_photo_filename_for(a_gd_photo_filenames_in_contact_directory, id, contact.photo.etag)) {
...
SyncFsmGd.prototype.gd_groups_in_zfi = function (zfi, is_only_groups_in_gid) {
    zinAssertAndLog(zfi.type() == FeedItem.TYPE_CN && zfi.styp() == FeedItem.eStyp.gdau, function () {
        return zfi.toString();
    });
...
SyncFsmGd.prototype.entryActionGetPuGd = function (state, event, continuation, type) {
    ...
    if (this.is_first_entry_to_state(state)) {
        ...
        let fn = function (sourceid, bucket) {
            return (sourceid == sourceid_pr) && (bucket == (Suo.MOD | feed_item_type));
        }
        let suo;
        ...
        if (type == GoogleData.eElement.contact) {
            fn = function (sourceid, bucket) {
                return (bucket == (Suo.ADD | feed_item_type));
            }
...
SyncFsmGd.prototype.exitActionGetPuGd = function (state, event, type) {
    ...
    if (!this.state.m_http.is_http_status(HTTP_STATUS_2xx)) {
        ...
        zinAssertAndLog(!(id in a_gd), function () {
            return "id=" + id;
        });
...
SyncFsmGd.gd_photo_filename_re_base = function (username) {
    return new RegExp(SyncFsmGd.gd_photo_filename_re_str_base(username));
}
SyncFsmGd.gd_photo_filename_re_full = function (username) {
    return new RegExp(SyncFsmGd.gd_photo_filename_re_str_base(username) + "\\.(\\w+?)");
}
