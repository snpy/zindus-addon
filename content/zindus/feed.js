...
function FeedItem() {
    ...
    if (arguments.length == 2) {
        ...
        zinAssertAndLog(FeedItem.ATTR_KEY in this.m_properties,
            function () {
                return " ATTR_KEY missing from: " + aToString(this.m_properties);
            });
...
FeedItem.prototype.set = function (arg1, arg2) {
    if (arguments.length == 2) {
        ...
        for each(var j
    in
        FeedItem.attributesForType(this.type())
    )
...
FeedItem.attributesForType = function (type) {
    ...
    switch (type) {
        case FeedItem.TYPE_CN:
            ret = ret.concat(FeedItem.ATTR_REV);
            break;
        case FeedItem.TYPE_FL:
            ret = ret.concat(FeedItem.ATTR_NAME, FeedItem.ATTR_PERM, FeedItem.ATTR_ACL);
            break;
        case FeedItem.TYPE_LN:
            ret = ret.concat(FeedItem.ATTR_NAME, FeedItem.ATTR_ZID, FeedItem.ATTR_RID);
            break;
        default:
            zinAssertAndLog(false, "unmatched case: " + type);
    }
